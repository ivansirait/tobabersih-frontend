...
export default function ManagePosts({ posts = [], onPostsUpdate }: ManagePostsProps) {
  ...
  const openModal = (post: any = null) => {
    ...
    if (post) {
      ...
      setEditingPost(null);
      setFormData({ title: '', slug: '', content: '', category: 'BERITA', imageUrl: '', author_id: 1 });
    }
    setError(null);
    setShowModal(true);
  };
  ...
  const validateForm = () => {
    if (!formData.title.trim()) { setError('Judul tidak boleh kosong'); return false; }
    if (!formData.content.trim()) { setError('Konten tidak boleh kosong'); return false; }
    return true;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingPost) {
        await axios.put(`/api/posts/${editingPost.id}`, dataToSend, config);
        toast.success('Berita berhasil diperbarui!');
      } else {
        await axios.post('/api/posts', dataToSend, config);
  ...
  const handleDelete = async (id: number) => {
    ...
    const token = localStorage.getItem('token');
    if (!token) { setError('Sesi Anda telah berakhir. Silakan login ulang.'); return; }
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-6">
      ...
      {/* FIX: Tutup div Header di sini — bukan di dalam flex gap-3 */}
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="PROFIL_DLH">Profil DLH</option>
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className="flex-1 bg-gradient-to-r from-green-500 to-green-600 text-white px-4 py-2 rounded-lg font-semibold hover:from-green-600 hover:to-green-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {loading ? 'Menyimpan...' : editingPost ? 'Perbarui' : 'Simpan'}
                  </button>
                </div>
              </form>
            </div>
